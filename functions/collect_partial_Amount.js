...
exports.collect_partial_Amount = async function (context, event, callback) {
  try {
    ...
    if (payment_amount >= 5 && payment_amount <= Number(Memory.userTotalBalance)) {
      ...
      Remember.say_err_msg = `You entered the payment amount $${payment_amount} is less than $5, we accept only amount $5 or more. Please Say or enter the amount again.`;
      Listen = false;
      Redirect = "task://payment_partial";

    }
    else if (payment_amount > Number(Memory.userTotalBalance)) {
      Remember.say_err_msg = `You entered the payment amount $${payment_amount} is more than your balance amount. Please Say or enter the amount again.`;
      Listen = false;
      Redirect = "task://payment_partial";
    }
